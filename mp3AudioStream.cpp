...
void mp3AudioStream::reloadFirstBuffers() {
  for (int i(1); i < BUFFER_MULTIPLIER; i++) {
    ...
    mpg123_read(mh, bufferDescriptors[i].buffer, maximumBufferSize,
                &decodedOffsetSize);
...
mp3AudioStream::mp3AudioStream(std::string filePath) : sf::SoundStream() {
  ...
  if (mh == nullptr) {
    std::cerr << "Handle initialisation goes wrong: "
              << mpg123_plain_strerror(err) << std::endl;
  }

  if (mpg123_open(mh, filePath.c_str()) != MPG123_OK ||
      mpg123_getformat(mh, &rate, &channels, &encoding) != MPG123_OK) {
    std::cerr << "Could not open or read file " << filePath << " : "
              << mpg123_plain_strerror(err) << std::endl;
    clearMemory();
    return;
  }
  ...
  std::cout << "Maximum size of one decoded frame buffer: " << maximumBufferSize
            << std::endl;
  std::cout << "Total number of sample for this track: " << totalSampleCount
            << std::endl;
  ...
  sharedBufferDescriptor.buffer =
      new sf::Int16[maximumBufferSize * BUFFER_MULTIPLIER];

  reloadFirstBuffers();
}

mp3AudioStream::~mp3AudioStream() { clearMemory(); }
...
bool mp3AudioStream::onGetData(Chunk &data) {
  ...
  if (bufferDescriptors[0].bufferSize * sizeof(sf::Int16) !=
      maximumBufferSize) {
  ...
  size_t decodedOffsetSize = 0;
  mpg123_read(mh, bufferDescriptors[BUFFER_MULTIPLIER - 1].buffer,
              maximumBufferSize, &decodedOffsetSize);
  bufferDescriptors[BUFFER_MULTIPLIER - 1].bufferSize =
      decodedOffsetSize / sizeof(sf::Int16);
